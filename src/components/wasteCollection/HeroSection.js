...
import { heroData } from "../../constants/WasteCollectionData";
      ...
      <div className="container mx-auto px-4 py-16 md:py-24 relative z-10">
        <div className="grid md:grid-cols-2 gap-12 items-center min-h-[70vh]">
          <div className="bg-white bg-opacity-10 backdrop-blur-md p-8 md:p-12 rounded-2xl border border-white border-opacity-20 shadow-2xl">
            ...
            <h1
              id="hero-title"
              className="text-3xl md:text-4xl lg:text-6xl font-bold leading-tight mb-8 text-white drop-shadow-lg"
            >
              {heroData.subtitle}
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <Link
                ...
                aria-label="Schedule a waste collection"
              >
                Schedule Collection
              </Link>
              <Link
              ...
              <div className="grid grid-cols-1 gap-4">
                {heroData.services.map((service, index) => (
                  <div key={index} className="flex items-center gap-4 bg-white bg-opacity-10 rounded-lg p-4 backdrop-blur-sm">
                    <span className="text-green-400 text-2xl" role="img" aria-label={service.text}>
                      {service.icon}
                    </span>
