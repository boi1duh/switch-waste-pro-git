...
import { COMMERCIAL_SECTORS } from '../../constants/CommercialBusinessesData';

const HeroSection = ({ onImageError, imageError }) => {
  const [bgImage, setBgImage] = useState('/assets/bins.jpg');

  const handleImageError = () => {
    setBgImage('/assets/fallback.jpg'); // Fallback image
          ...
          <div className="bg-white bg-opacity-10 backdrop-blur-md p-8 md:p-12 rounded-2xl border border-white border-opacity-20 shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <Link
                ...
                aria-label="Get a commercial quote"
              >
          ...
          <div className="hidden md:block">
            <div className="bg-white bg-opacity-10 backdrop-blur-md p-8 rounded-2xl shadow-2xl max-w-lg border border-white border-opacity-20">
              ...
              <div className="grid grid-cols-1 gap-4" role="list">
                {COMMERCIAL_SECTORS.map((sector, index) => (
                  <div key={index} className="flex items-center gap-4 bg-white bg-opacity-10 rounded-lg p-4 backdrop-blur-sm" role="listitem">
                    <span className="text-blue-400 text-2xl" aria-hidden="true">{sector.icon}</span>
                    <span className="text-white text-lg font-medium drop-shadow-md">{sector.name}</span>
                  </div>
                ))}
              </div>
            </div>
