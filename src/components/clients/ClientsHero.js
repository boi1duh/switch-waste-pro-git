...
import { partnershipBenefits } from "../../constants/ClientsData";
              ...
              <div className="grid grid-cols-1 gap-4">
                {partnershipBenefits.map((benefit, index) => (
                  <div key={index} className="flex items-center gap-4 bg-white bg-opacity-10 rounded-lg p-4 backdrop-blur-sm">
                    <span className="text-green-400 text-2xl">{benefit.icon}</span>
                    <span className="text-white text-lg font-medium drop-shadow-md">{benefit.text}</span>
                  </div>
                ))}
              </div>
            </div>
