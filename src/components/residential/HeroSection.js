...
import { HERO_DATA } from "../../constants/ResidentialData";
...
const HeroSection = React.memo(() => {
  ...
  return (
    <section
      ...
      role="banner"
      aria-label="Residential waste solutions hero section"
    >
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center min-h-[70vh]">
          ...
          <div className="hidden md:block">
            <div className="bg-white bg-opacity-10 backdrop-blur-md p-8 rounded-2xl shadow-2xl max-w-lg border border-white border-opacity-20">
              <h3 className="font-bold text-white mb-6 text-2xl drop-shadow-lg">
              ...
              <div className="grid grid-cols-1 gap-4">
                {HERO_DATA.services.map((service, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-4 bg-white bg-opacity-10 rounded-lg p-4 backdrop-blur-sm"
                  >
                    <span className="text-green-400 text-2xl" role="img" aria-label={service.label}>
                      {service.icon}
                    </span>
