...
import { HERO_DATA } from "../../constants/ControlledSubstanceWasteData";
...
const HeroSection = () => {
  return (
    <section
      ...
      aria-label="Hero section for Controlled Substance Waste services"
    >
          ...
          <div className="bg-white bg-opacity-10 backdrop-blur-md p-8 md:p-12 rounded-2xl border border-white border-opacity-20 shadow-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <Link
                ...
                aria-label="Get a quote for controlled substance waste services"
              >
                Get Quote
              </Link>
              <Link
          ...
          <div className="hidden md:block">
            <div className="bg-white bg-opacity-10 backdrop-blur-md p-8 rounded-2xl shadow-2xl max-w-lg border border-white border-opacity-20">
              ...
              <div className="grid grid-cols-1 gap-4">
                {HERO_DATA.controlledSubstances.map((item, index) => (
                  <div key={index} className="flex items-center gap-4 bg-white bg-opacity-10 rounded-lg p-4 backdrop-blur-sm">
                    <span className="text-red-400 text-2xl" role="img" aria-label={item.text}>{item.icon}</span>
